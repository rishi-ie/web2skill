...
1. Confirm on route `/:team/issue/:id`
2. Click `property-status` in the sidebar
3. In the popover, click "Done"
4. Verify: `issue-status` badge changes to "Done" (or
   "Completed"), a toast appears bottom-right reading
   "Marked as Done"
5. If the team is configured to auto-archive completed issues,
   the issue is moved to the Completed tab

### Change the assignee

1. Confirm on route `/:team/issue/:id`
2. Click `property-assignee` in the sidebar
3. In the popover, type the assignee's name in the search
   input
4. Click the user from the results
5. Verify: `issue-assignee` avatar changes to the new user

### Add a comment

1. Confirm on route `/:team/issue/:id`
2. Scroll to `comment-input` at the bottom of the comments
   list
3. Type the comment text (supports markdown and `@mentions`)
4. Click `comment-submit` (verify it is enabled)
5. Verify: the new `comment` appears at the bottom of
   `comments-list` with a "just now" timestamp

### Subscribe to updates

1. Confirm on route `/:team/issue/:id`
2. Click `subscribe-btn` in the sub-header
3. Verify: button icon fills in, label changes to
   "Unsubscribe"
4. The user will now receive inbox notifications for changes
   to this issue

### Delete the issue

1. Confirm on route `/:team/issue/:id`
2. Click `more-menu` in the sub-header
3. Click "Delete" in the dropdown
4. A confirmation modal appears: "Delete this issue? This
   cannot be undone."
5. Click "Delete issue" in the modal
6. Verify: URL changes to `/:team`, the issue is gone from
   all tabs
