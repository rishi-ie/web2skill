...
### Create a new issue with a title only

1. Confirm on route `/:team/issue/new` (modal is open)
2. Type the issue title into `title-input`
3. Click `create-btn` (verify it is enabled)
4. Verify: modal closes, URL changes to `/:team/issue/<new-id>`,
   the new issue is shown (load `issue.md`)

### Create a new issue with full details

1. Confirm on route `/:team/issue/new` (modal is open)
2. Type the issue title into `title-input`
3. Type the issue description (markdown) into
   `description-input`
4. Set `status-select` to the desired status (default is
   Backlog)
5. Set `priority-select` to the desired priority (default is
   No priority)
6. Optionally set `assignee-select` and `labels-select`
7. Click `create-btn`
8. Verify: modal closes, URL changes to
   `/:team/issue/<new-id>`, the new issue shows all the
   properties you set

### Cancel creating an issue

1. Confirm on route `/:team/issue/new` (modal is open)
2. Click `cancel-btn` (or `modal-close`, or press Escape, or
   click outside the modal on `modal-overlay`)
3. Verify: modal closes, URL changes back to `/:team` (the
   sub-tab you were on before)
