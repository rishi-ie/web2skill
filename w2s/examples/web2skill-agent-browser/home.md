...
- **Header (inherited from overview):** logo + nav (Pricing, Sign up, Log in)
- **Hero (top of main column):** headline, subhead, two CTAs ("Get started" → `/signup`, "View pricing" → `/pricing`)
- **Features section:** three feature blocks in a row, each with an icon, title, and short description
- **Testimonial section:** a single pull quote from a customer
- **Final CTA (bottom of main column):** "Ready to get started?" with a "Sign up free" button
...
- **location:** hero section, right side, secondary button (left of `hero-cta-signup`)
...
1. Confirm on route `/`
2. Click `nav-pricing` (or `hero-cta-pricing` from the hero section)
3. Verify: URL changes to `/pricing`, page shows pricing content

### Navigate to signup

1. Confirm on route `/`
2. Click `nav-signup` (or `hero-cta-signup` from the hero, or `final-cta-signup` from the bottom CTA)
...
agent-browser url
# Should be https://harvestgrove.com/pricing
```
